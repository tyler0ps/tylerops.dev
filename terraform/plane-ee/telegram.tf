...
resource "aws_ssm_parameter" "telegram_webhook_secret" {
  name  = "/plane-ee/telegram/webhook-secret"
  type  = "SecureString"
...
resource "aws_ssm_parameter" "telegram_bot_token" {
  name  = "/plane-ee/telegram/bot-token"
  type  = "SecureString"
...
resource "aws_ssm_parameter" "telegram_chat_id" {
  name  = "/plane-ee/telegram/chat-id"
  type  = "String"
...
resource "aws_lambda_function" "telegram_bot" {
  function_name = "plane-ee-telegram-bot"
  description   = "Telegram bot for Plane EE ASG control"
  ...
  environment {
    variables = {
      ...
      EIP_ADDRESS              = aws_eip.plane_ee.public_ip
      LAMBDA_LOG_GROUP         = aws_cloudwatch_log_group.telegram_lambda.name
    }
  }

  tags = {
    Name = "plane-ee-telegram-bot"
  }
}

resource "aws_cloudwatch_log_group" "telegram_lambda" {
  name              = "/aws/lambda/plane-ee-telegram-bot"
  retention_in_days = 7

  tags = {
    Name = "plane-ee-telegram-bot-logs"
  }
}
...
resource "aws_iam_role" "telegram_lambda" {
  name = "plane-ee-telegram-lambda-role"
...
resource "aws_iam_role_policy" "telegram_lambda_permissions" {
  name = "plane-ee-telegram-lambda-permissions"
