"""
Plane EE Graceful Shutdown Lambda

Handles ASG lifecycle hook termination events:
- Gracefully stops Plane EE services via SSM
...
def graceful_shutdown(instance_id):
    """Execute graceful shutdown via SSM (stop Plane EE services)."""
    ...
    try:
        response = ssm.send_command(
            ...
            Parameters={
                "commands": [
                    "script -qec 'prime-cli stop' /dev/null || true",
                    "sleep 10",
                ]
            },
            TimeoutSeconds=60,
