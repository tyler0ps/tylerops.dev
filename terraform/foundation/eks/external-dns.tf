...
locals {
  ...
  external_dns_oidc_url        = trimprefix(module.eks_karpenter.cluster_oidc_issuer_url, "https://")
}
...
resource "aws_iam_role" "external_dns" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${local.external_dns_oidc_url}:sub" = "system:serviceaccount:${local.external_dns_namespace}:${local.external_dns_service_account}"
          "${local.external_dns_oidc_url}:aud" = "sts.amazonaws.com"
        }
      }
    }]
  })
